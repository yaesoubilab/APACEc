...
namespace APACElib
{
    public abstract class GonoModel : ModelInstruction
    {
        ...
        double[] PrevFeasibleRange = new double[2] { 0.005, 0.15 }; // { 0.001, 0.2 }
        double[] PercSympFeasibleRange = new double[2] { 0.5, 0.9 }; // { 0.5, 0.9 }
        double[] CaseRateFeasibleRange = new double[2] { 0.005, 0.15 };  // { 0.0, 0.2 }
        ...
        protected void AddGonoFeatures(List<string> regions)
        {
            ...
            foreach (ResistStates r in Enum.GetValues(typeof(ResistStates))) // G_0, G_A, G_B, G_AB
            {
                if (r != ResistStates.G_0)
                {
                    _featureInfo.IDPercResist[(int)r] = id;
        ...
        protected void AddGonoConditions(List<string> regions)
        {
            ...
            // turn off B
            c1 = _conditionInfo.ConditionIDs[(int)Conditions.BOut];
            c2 = _conditionInfo.ConditionIDs[(int)Conditions.ABOut];
            _epiHist.Conditions.Add(new Condition_OnConditions(
                id: id++,
            ...
            // turn on M
            c1 = _conditionInfo.ConditionIDs[(int)Conditions.BOff];
            _epiHist.Conditions.Add(new Condition_OnConditions(
                id: id++,
                name: "Drug M - Turn On",
                conditions: new List<Condition> {
